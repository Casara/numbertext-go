...
**Status**: not yet submitted upstream. To contribute it:
[open an issue or PR against `Numbertext/libnumbertext`](https://github.com/Numbertext/libnumbertext)
with this diff against `data/bg.sor`. Once merged upstream and picked
up by a future `scripts/sync-data.sh` run, remove this section and the
matching re-patch step in that script.
